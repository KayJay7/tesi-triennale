...
\paragraph{Protocollo di GoAbU in breve} Ogni volta che viene attivata ua regola esterna, GoAbU inizia una ``transazione''. Una transazione è composta da una sequenza di ``azioni'', ovvero dei messaggi ad alto livello inviati \emph{in unicast} tramite memberlist.
Queste azioni conterranno il mittente, il tipo di azione (ad esempio \lstinline{"do_commit"}, od \lstinline{"interested"}), ed il corpo dell'azione (ad esempio, l'albero sintattico di un'aggiornamento di una variabile). L'azione è serializzata in \lstinline{json} e successivamente in \lstinline{base64}.\\
Una transazione si svolge in più fasi. Prima, il nodo che ha attivato la regola (l'iniziatore) esegue un'azione per tutti i nodi che conosce, per scoprire se sono interessati (anche la risposta è un'azione da parte del destinatario).
Poi un nuovo scambio di azioni invierà il comando da eseguire sulle variabili a tutti i nodi interessati, che successivamente decideranno se abortire o eseguire il comando, ed informeranno l'iniziatore della decisione. Se un nodo abortisce viene comunicato anche agli altri, altrimenti viene comunicato di eseguirlo.
Se un nodo non risponde entro un certo timeout, l'iniziatore ripete le azioni.
...
Il parsing delle regole di ogni nodo viene fatto direttamente dal simulatore, che dopo aver costruito l'elenco di regole e variabili di ogni nodo, codificherà gli alberi di sintassi in \lstinline{json} e successivamente in \lstinline{base64} per passarli come argomenti al programma in esecuzione su ogni nodo, proprio come sono codificate le azioni. È possibile collegare altri nodi esterni alla rete del simulatore che eseguano GoAbU, ma non è necessario per questa tesi.\\
...
Inoltre, abbiamo deciso di raccogliere anche il numero di azioni eseguite a livello di libreria, e la dimensione di queste azioni. Per poterle confrontare con i dati del sistema operativo e scoprire quando overhead viene aggiunto dal middleware.
...
I dati sulle azioni, invece, sono raccolte tramite i log del simulatore, che sono stati modificati appositamente per includere questi dati.
Purtroppo, per quanto riguarda le azioni, abbiamo raccolto i log di un solo nodo poiché le dimensioni erano troppo grandi. Raccogliendo i dati di un solo nodo sono stati prodotti $\qty{230}{\mebi\byte}$ di file di log, quindi raccoglierli per tutti i nodi sarebbe stato dispendioso.
...
\begin{itemize}
    ...
    \item \lstinline{logs_expected bytes}: Dimensione totale delle azioni riportata nei log
    \item \lstinline{logs_actual bytes}: Byte effettivamente inviati dal nodo loggato
    \item \lstinline{logs_expected packets}: Numero di azioni riportata nei log
    ...
    \item \lstinline{logs_packets ratio}: Rapporto tra pacchetti inviati e azioni riportate nel log
\end{itemize}
...
Il secondo invece mette in evidenza quanti pacchetti servono per un'azione.
...
È calcolato confrontando la quantità byte dati inviati riportata dal sistema, con la somma della dimensione di tutte le azioni eseguite a dalla libreria. Quindi un valore maggiore di $1$ indica un complessivo overhead, mentre un valore minore di $1$ indica una compressione migliore.
...
\begin{figure}[H]
    ...
    \caption{Rapporto tra pacchetti effettivamente inviati ed azioni riportate nel log}
    \label{fig:lpr}
\end{figure}

Questa metrica mette in evidenza quanti pacchetti sono \emph{inviati} (non scambiati in totale), approssimativamente, per ogni azione.
...
Possiamo vedere che il numero di pacchetti per azione è influenzato principalmente dal tipo di carico. Comunque, anche sotto carico alto, sono meno di $4$ pacchetti per azione.
...
Alcuni nodi ricevono più o meno traffico. Anche se le azioni sono in broadcast e dovrebbero essere distribuite tra tutti i nodi.
...
\begin{figure}[H]
    ...
    \caption{Dimensione totale delle azioni riportata nei log}
    \label{fig:leb}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Numero di azioni riportata nei log}
    \label{fig:lep}
\end{figure}
