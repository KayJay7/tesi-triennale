...
\michele{Manca un sezione introduttiva, non serve praticamente a molto ma formalmente ci dovrebbe essere, magari riduci un pò l'abstract e metti una sua versione estesa qui, a fine sezione poi metti una sinopsi in cui descrivi la struttura della tesi ed in due parole le varie sezioni cosa contengono}
...
Le regole non sono molto diverse da delle normali ECA: ogni regola è legata ad una variabile del nodo \michele{una o più}, e quando questa variabile viene modificata (da una regola o da un dispositivo di input), se una condizione specificata è verificata, esegue l'azione.
...
In questo modo, è possibile programmare un sistema distribuito completamente disaccoppiato nello spazio, poiché l'effetto di un'azione sugli altri nodi dipende dai loro attributi e non richiede di conoscere la posizione delle variabili. \michele{Ho messo il pattern della regola centrato, per dare un pò di respiro, sistema anche quella dopo}
Una regola interna AbU è della forma:
\begin{center}
\lstinline{rule <name> on <variable> for <condition> do <action>}
\end{center}
Ad esempio \lstinline{rule step0_0 on a0_0 for a0_0 > 0 do a0_1 = a0_0; a0_0 = 0}. 

\michele{Magari fai esempi un pò più ``reali'', e spiega in due parole cosa fanno} Una regola esterna, invece, è della forma \lstinline{rule <name> on <variable> for all <condition> do <action>} (notare \lstinline{for all} al posto di \lstinline{for}) e può accedere a variabili remote (non per l'attivazione), ad esempio \lstinline{rule activate0 on a0_4 for all this.a0_4 > 0 && (ext.id == 8) do ext.a0_0 = (this.a0_4 - 1)}.
...
Entrambi questi problemi non rientrano nell'obbiettivo della tesi, ma sono venuti alla luce perché per la prima volta il calcolo AbU è \michele{stato adottato in un contesto applicativo.}%stato usato per qualcosa al di fuori di uno showcase. Quindi era il caso di esporli.
...
\michele{Metterei anche una sottosezione con i dettagli della macchina sulla quale sono stati effettuati i test}
...
Una soluzione proposta e testata in un fork del simulatore prevede di non passare la configurazione come argomento, ma attraverso il buffer di input, che invece non è limitato. Questa soluzione funziona e può essere implementata semplicemente, ma non è l'unica possibilità. \michele{Ma è quella che hai usato negli esperimenti?}
...
Un rule engine è efficiente a trovare le regole da eseguire perché utilizza una struttura dati complessa, ma per costruirla deve prima eseguire una computazione complessa. E Grule non fa eccezione, costruire la struttura dati dal set di regole è costoso, sembra richiedere costo $\Theta(x^2)$, \michele{Da dove arriva questa complessità?} quindi avviando il simulatore su una grande quantità di regole (non necessario per questa tesi) potrebbe passare molto tempo prima che i nodi siano completamente avviati.
...
Per questa tesi sono rilevanti solo i 6 parametri di configurazione, in quanto il setting sperimentale è rimasto lo stesso in ogni test. \michele{Il valore del tick?}
...
\michele{La chiamerei stabilizzazione e non convergenza}
...
Come annunciato all'inizio della sezione~\ref{terminazione:premesse:esterna}, affronteremo la convergenza esterna considerando corretta la parte interna della computazione. Ovvero assumendo che nodo che viene ``attivato'' da un valore $k$, eventualmente attiverà $e$ nodi con un valore $k-1$. Questo segue dalla dimostrazione precedente~\ref{terminazione:interna}.
...
\paragraph{Idle}
\michele{Metteri l'elenco dopo la descrizione (anche per i successivi)}
\begin{itemize}
    \item $a=10|50|100$
    \item $b=1$
    \item $c=1$
...
L'obbiettivo era \michele{non?} essere troppo pesante sulla rete, ma bilanciare il carico tra rete e nodo interno. In un certo tempo, ogni nodo avrà molte più regole locali di cui occuparsi, e dovrebbe utilizzare di meno la rete.
...
\michele{La tabella è poco chiara, la prima colonna è il numero di nodi, mentre la seconda e la terza sono il numero di regole eseguite, farei un mutlicolumn, dividendo poiin interne ed esterne (anche per la successiva)}
...
L'ultimo test è stato progettato con l'obbiettivo esplicito di mettere in difficoltà la rete quanto più possibile in un tempo limitato. Come si vede dalla configurazione, non è presente nessuna regola interna \michele{nessun regola interna è un pò misleading, visto che dalla tabella si vede che comunque molte regole interne vengono attivate}se non quelle strettamente necessarie.
